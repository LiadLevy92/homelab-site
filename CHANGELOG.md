...
- **וידאו 2 (הדגמה בשטח, מטושטש) — קליטת Wi-Fi** — נוסף סרטון שני אחרי ה-callout: מחברים את האנטנה ורשימת רשתות ה-Wi-Fi הזמינות גדלה ניכרת (הוכחה בשטח). קוצר מ-67ש' ל-25ש' (קטע רציף 12s→37s: בסיס→חיבור→רשימה שגדלה), `live-demo-wifi.mp4` (~1.1MB, 480×854).
  - **פרטיות (טשטוש מבוקר-זמן):** `gblur=sigma=8` **מטושטש על כל קטעי המסך** (שמות הרשתות/SSID של השכנים אינם קריאים, אך עדיין רואים שמספר השורות גדל), ו**חד רק בחלון 8–14ש'** שמראה את האנטנה הפיזית — `enable='not(between(t,8,14))'`. אומת frame-by-frame מהפלט: מסכים מטושטשים, חלון האנטנה חד, אין דליפת מסך קריא. הכיתוב מציין שהשמות טושטשו לפרטיות.
